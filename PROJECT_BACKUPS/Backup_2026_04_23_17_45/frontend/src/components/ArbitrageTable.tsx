...
import { getBlackMarketFlips, ArbitrageOpportunity } from '@/lib/api';
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      try {
        ...
        const response = await fetch(`https://www.albion-online-data.com/api/v2/stats/prices/${itemIds}?locations=Lymhurst,Bridgewatch,Martlock,Thetford,Fort Sterling,Caerleon`);
    ...
    const checkRole = async () => {
      if (supabase) {
        ...
        if (currentUser) {
          ...
          if (error) {
            // Suppressing error to avoid Next.js overlay
          }
          
          const hasProAccess = ['pro', 'elite', 'owner', 'founder', 'admin'].includes(profile?.role);
    ...
    if (supabase) {
      const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
        ...
        if (currentUser) {
          ...
          setIsPro(['pro', 'elite', 'owner', 'founder', 'admin'].includes(profile?.role));
        } else {
          setIsPro(false);
        }
      });
      ...
      <div className="px-6 py-6 border-b border-white/10 flex flex-wrap items-end gap-6 bg-white/5">
        ...
        <div className="flex items-center gap-3 ml-auto mb-1">
          <button 
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-white/5 border border-white/5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/20 transition-all group shadow-lg"
          >
            <RefreshCw className="w-3.5 h-3.5 group-hover:rotate-180 transition-transform duration-500" />
            Reset All
          </button>
          
          <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 bg-slate-950/50 px-5 py-2.5 rounded-xl border border-white/5 shadow-xl">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.15em] bg-white/[0.02] border-b border-white/10">
              ...
              <th className="px-6 py-4 text-center text-slate-600">AGE</th>
            </tr>
          </thead>
            ...
            {filteredOps.map((op, idx) => {
              ...
              return (
                <React.Fragment key={`${op.item_id}-${op.quality}-${idx}`}>
                  ...
                  <td className="px-6 py-6">
                    <div className="flex items-center gap-5">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-3">
                          <div className={`font-semibold text-lg tracking-tight transition-colors flex items-center gap-2 ${isRestricted ? 'text-amber-500/40 select-none' : isPrivate ? 'text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]' : 'text-slate-100 group-hover:text-amber-400'}`} style={{ fontFamily: "'Outfit', sans-serif" }}>
                            {isRestricted ? `TIER ${op.item_id.charAt(1)} PREMIUM ITEM` : inGameName}
                            {!isRestricted && isEnchanted && <span className="text-amber-500/80 ml-1">.{enchantLevel}</span>}
                            {isRestricted && (
                            ...
                            {isPrivate && !isRestricted && (
                              <span className="ml-2 px-2 py-0.5 bg-gradient-to-r from-amber-400 to-amber-600 text-slate-950 text-[9px] font-black rounded uppercase tracking-widest shadow-[0_0_15px_rgba(245,158,11,0.8)] animate-pulse flex items-center gap-1">
                          ...
                          {!isRestricted && (
                            <button 
                              onClick={() => copyToClipboard(inGameName.split(' ').slice(1).join(' '))}
                              className="opacity-0 group-hover:opacity-100 p-2 bg-white/5 hover:bg-white/10 rounded-lg transition-all"
                              title="Copy Name"
                            >
                              <svg className="w-4 h-4 text-slate-400 hover:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                            </button>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <span 
                            className={`inline-block text-[11px] font-black px-2.5 py-0.5 rounded tracking-wider uppercase ${isRestricted ? 'bg-amber-500/5 text-amber-500/30 border border-amber-500/10' : ''}`}
                            style={!isRestricted ? {
                  ...
                  <td className="px-6 py-6">
                    <div className={`text-lg font-bold transition-all flex items-center justify-center gap-1.5 ${isRestricted ? 'opacity-30' : 'text-slate-200'}`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {isRestricted ? (
                  ...
                  <td className="px-6 py-6">
                    <div className={`text-lg font-bold transition-all flex flex-col items-center gap-1 ${isRestricted ? 'opacity-30' : 'text-slate-200'}`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      ...
                      {!isRestricted && op.demand !== undefined && op.demand > 0 && (
                        <div className="px-2 py-0.5 bg-amber-500/10 text-amber-500 text-[9px] font-black uppercase rounded border border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.15)] mt-1 animate-in fade-in slide-in-from-top-1">
                          Qty {op.demand}
                        </div>
                      )}
                    </div>
                  ...
                  <td className="px-6 py-6">
                    <div className={`flex flex-col items-center gap-2 transition-all ${isRestricted ? 'blur-md opacity-20' : ''}`}>
                      ...
                      <div className="flex flex-wrap justify-center gap-1 max-w-[120px]">
                        ...
                        {op.item_id.includes('@') ? (
                          <span className="px-1.5 py-0.5 bg-purple-500/10 text-purple-400 text-[7px] font-black uppercase rounded-sm border border-purple-500/20">Enchant</span>
                        ) : null}
                      </div>
                    </div>
                  </td>
